 ...
 * @since 9/1/2026
 */
public record SuperCrafterRecipe(Ingredient upgradeScroll, Ingredient hammer, Ingredient stageItem, ItemStackTemplate output, int hammerUsage) implements Recipe<SuperCrafterRecipeInput> {

    public static final MapCodec<SuperCrafterRecipe> CODEC = RecordCodecBuilder.mapCodec(instance ->
            instance.group(
                    Ingredient.CODEC.fieldOf("update_scroll").forGetter(SuperCrafterRecipe::upgradeScroll),
                    ...
                    Codec.INT.fieldOf("hammer_usage").forGetter(SuperCrafterRecipe::hammerUsage)
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, SuperCrafterRecipe> STREAM_CODEC = StreamCodec.composite(
            Ingredient.CONTENTS_STREAM_CODEC, SuperCrafterRecipe::upgradeScroll,
            ...
            ByteBufCodecs.VAR_INT, SuperCrafterRecipe::hammerUsage,
            SuperCrafterRecipe::new);

    @Override
    public boolean matches(SuperCrafterRecipeInput input, Level level) {
        if (level.isClientSide()) return false;
        return this.upgradeScroll.test(input.getItem(SuperCrafterRecipeInput.Context.TEMPLATE_POSITION))
